...
4. 对于剩余词及词组合，匹配知识库中已经分好类的实体。如果没有匹配到实体，或者匹配到的实体属于0类（即非实体），则将其过滤掉。 
5. 实体的分类算法见下文。
...
- 预测一个页面时，由于KNN要将该页面和训练集中所有页面进行比较，因此每次预测的复杂度是O(n)，n为训练集规模。
   在这个过程中，我们可以统计各个分相似度的IDF值，均值，方差，标准差，然后对4个相似度进行标准化:**(x-均值)/方差**
...
##############################################
1.获取实体：分词、词性标注，根据标注结果，选出名词、人名等作为实体
2.对实体进行分类：根据农业实体类别，将获取的实体进行分类
  （1、2部可直接用实体识别算法代替）
3.实体关系抽取：pcnn
4.实体统一、指代消解
5.知识存储，neo4j数据库